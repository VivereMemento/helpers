...
Maybe(url.split('#').length > 1)
	.map(
		compose(
			replacedHashInSlideLink,
			replacedUrlWithQuestion,
			replacedPageWithQuestion('data-next-post-new-link', 'nextPostNewLink'),
			replacedPageWithQuestion('data-next-post-link', 'nextPostLink')
		)
	)
